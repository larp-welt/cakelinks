 ...
 */
	function url($url = null, $full = false) {
		...
		if (is_array($url)) {
			...
			// ME: check prefixes other then admin routing
			...
			// ME: routes are no named parameters
			$skip = array_merge($skip, $_this->__prefixes);
			...
			if ($match === false) {
				...
				// ME: other Routes than admin route
				foreach ($_this->__prefixes as $prefix) {
					if (!empty($url[$prefix])) {
				...
				// ME: routes other than admin
				foreach ($_this->__prefixes as $prefix) {
					if (isset($url[$prefix])) array_unshift($urlOut, $prefix);
				}
				$output = join('/', $urlOut) . '/';
			}
